...
# def test_strerror():
#     # idn2_strerror: Converts an internal error code to a human-readable error message.
#     result = pydn2.strerror(-1)
#     assert isinstance(result, str)
#     assert len(result) > 0

# def test_strerror_name():
#     # idn2_strerror_name: Converts an internal error code to its corresponding error name.
#     result = pydn2.strerror_name(-1)
#     assert isinstance(result, str)
#     assert len(result) > 0

# Emoji tests with "😀domain" - using the NFC flag
@pytest.mark.focus
